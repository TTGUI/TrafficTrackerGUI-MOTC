...
class StableVideo:
    ...
    def ECC(self, x) :
        ecc_cap_s = time.time()
        cap = cv2.VideoCapture(os.path.join( os.path.abspath(self.inputVideoPath), self.videolist[self.currentIndex]))
        ...
        if cap.isOpened():
            ...
            ecc_ecc_s = time.time()
            print("ECC_CAP : " + str(ecc_ecc_s - ecc_cap_s))
            ...
            print("ECC_ECC : " + str(time.time() - ecc_ecc_s))
            cap.release()
            ...
            if self.cutInfoList[self.currentIndex].getKey() == -1 and self.cutInfoList[self.currentIndex].getStart() == -1 and self.cutInfoList[self.currentIndex].getEnd() == -1 :
                ...
                with multiprocessing.Pool(multi) as pool :
                    ...
                    while bigWhile > 0 :
                        ...
                        ECC_S = time.time()
                        result = pool.map_async(self.ECC, waitinput)
                        WRI_S = time.time()
                        print("ALL_ECC : " + str(time.time() - ECC_S))
                        output =  result.get()              
