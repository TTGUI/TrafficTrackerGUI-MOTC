...
if __package__.find('YOLOv4') != -1:
    from .torch_utils import init_seeds, is_parallel
    if os.name == 'nt':
        from .box_utils_win.iou_rotate import iou_rotate_calculate1, iou_rotate_calculate2
        from .box_utils_win.nms_rotate import nms_rotate_cpu
        from .box_utils_win.rotate_polygon_nms import rotate_gpu_nms
    else:
        from .box_utils_linux.iou_rotate import iou_rotate_calculate1, iou_rotate_calculate2
        from .box_utils_linux.nms_rotate import nms_rotate_cpu
        from .box_utils_linux.rotate_polygon_nms import rotate_gpu_nms
...
def compute_loss(p, targets, model, batch_No=-1, num_iter=-1, epoch=-1, theta_format='dhxdhy', loss_terms='hioudhxdhy', num_extra_outputs=0, n_loss_terms=3, tb_writer=None, imgs=None, cv_imgs=None, log_dir='', use_angle_modify_factor=False):  # predictions, targets, model
    """
    default:
        train_mode='dxdy'
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            # print('pxy dtype', pxy.dtype)
            # print('pwh dtype', pwh.dtype)
            ...
            # rotate iou loss
                ...
                if loss_terms.find('giou') != -1:
                    giou = oriented_iou_loss.cal_giou(pxywhtheta, txywhtheta)[0]
                elif loss_terms.find('diou') != -1:
                    giou = oriented_iou_loss.cal_diou(pxywhtheta, txywhtheta)[0]
                elif loss_terms.find('iou') != -1:
                    # print('iou loss~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
...
def coco_single_class_labels(path='../coco/labels/train2014/', label_class=43):
    ...
    for file in tqdm(sorted(glob.glob('%s/*.*' % path))):
        ...
        if any(i):
            ...
            with open('new/images.txt', 'a', encoding='utf-8') as f:  # add image to dataset list
                f.write(img_file + '\n')
            with open('new/labels/' + Path(file).name, 'a', encoding='utf-8') as f:  # write label
                for l in labels[i]:
...
def kmean_anchors(path='./data/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    if isinstance(path, str):  # *.yaml file
        ...
        from utils_pkg.datasets import LoadImagesAndLabels
...
def save_and_plot_valid_result_to_tensorboard(f, preds, conf_thres, theta_format, num_extra_outputs, img, save_dir, epoch, tb_writer):
    ...
    if x.shape[0] > 0:
        ...
        for idx, pts in enumerate(rbox_4xy):
            if nms_out: draw_one_polygon(img, pts.cpu(), int(x[idx,6]), rgb_color=True, line_thickness=1)
            else: draw_one_polygon(img, pts, 0, True, 1)

    if os.name == 'nt':
        cv2.imencode(ext='.jpg',img=img)[1].tofile(str(save_dir)+os.sep+f)
    else:
...
def save_and_plot_anchors_on_img_to_tensorboard(model, log_dir, num_iter, epoch, cv_imgs, indices, tbox, anchors, offsets, theta_format, extra_outs, tb_writer):
    ...
    for img_id, img in enumerate(cv_imgs):
        ...
        for i, anchor_stride in enumerate(anchor_stride_list):
            ...
            if os.name == 'nt':
                cv2.imencode(ext='.jpg',img=img_show_one_anchor)[1].tofile(yolo_layer_anchor_filename)
            else:
                cv2.imwrite(yolo_layer_anchor_filename, img_show_one_anchor)
            
    if num_iter == 0:
        if os.name == 'nt':
            cv2.imencode(ext='.jpg',img=img_show_all_anchors)[1].tofile(all_anchors_filename)
        else:
            cv2.imwrite(all_anchors_filename, img_show_all_anchors)
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16, theta_format='dhxdhy', num_extra_outputs=0, plotontb=False, isGT=True):
    ...
    if fname is not None:
        ...
        if os.name == 'nt':
            cv2.imencode(ext='.jpg',img=cv2.cvtColor(mosaic, cv2.COLOR_BGR2RGB))[1].tofile(fname)
        else:
...
def plot_labels(labels, save_dir=''):
    ...
    with open(str(save_dir)+os.sep+'label_class_count.txt', 'w', encoding='utf-8') as outfile:
