...
if __package__.find('YOLOv4') != -1:
    from .general import xyxy2xywh, xywh2xyxy, fourxy2xywh, xywhtheta24xy_new, \
        torch_distributed_zero_first, get_number_of_extra_outputs
    from .extractRect import extract_rect_from_poly    
else:
    from general import xyxy2xywh, xywh2xyxy, fourxy2xywh, xywhtheta24xy_new, \
        torch_distributed_zero_first, get_number_of_extra_outputs
    from extractRect import extract_rect_from_poly    
            ...
            if l.shape[0]:
                ...
                if create_datasubset and ns < 1E4:
                    ...
                    if exclude_classes not in l[:, 0]:
                        ...
                        with open('./datasubset/images.txt', 'a', encoding='utf-8') as f:
...
def reduce_img_size(path='path/images', img_size=1024):  # from utils.datasets import *; reduce_img_size()
    ...
    for f in tqdm(glob.glob('%s/*.*' % path)):
        try:
            ...
            if os.name == 'nt':
                cv2.imencode(ext='.png',img=img)[1].tofile(fnew)
            else:
    ...
    for a, b, files in os.walk(dataset):
        for file in tqdm(files, desc=a):
            ...
            if s == '.txt':  # replace text
                ...
                if os.name == 'nt':
                    frame = cv2.imdecode(np.fromfile(p,dtype=np.uint8),-1)
                    frame = cv2.cvtColor(frame,cv2.COLOR_RGB2BGR)
                    cv2.imencode(ext='.bmp',img=frame)[1].tofile(p.replace(s, '.bmp'))
                else:
