...
def remove_duplicate_box(path, iou_thres=0.8):
    ...
    for frame_token in tokens:
        ...
        output_8cls_str += str(frame_id)
        for obj_info in objs_info_np:
            pts = obj_info[2:].astype(int)
            output_8cls_str += ' ' + str(int(obj_info[0])) + ' ' + '{:.2f}'.format(obj_info[1]) \
                                + ' ' + str(pts[0]) + ' ' + str(pts[1]) \
    ...
    with open(path, 'w') as outfile:
        outfile.writelines(output_8cls_str)

if __name__ == '__main__':
    path = r"D:\UAV_traffic_conflict_analysis_300\TrafficTrackerGUI-MOTC\result\實例\OR\彰化縣公園路一段_中山路二段路口120米_B_stab_8cls - 複製.txt"
    newP = Path(path)
