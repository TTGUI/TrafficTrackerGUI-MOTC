...
from .utils_pkg.datasets import LoadStreams, LoadImages
...
#     if save_dota_txt:
#         dota_detect_cls_txt_dir = os.getcwd() + os.sep + 'dota_detect_clstxt'
...
#                     draw_one_polygon(img_before_nms, 0, *pts)
...
#                         draw_one_polygon(im0, int(cls), *xyxy, line_thickness=2)
...
def obb_object_detect(source_path='', output_detect_result_path='', yolo_model=''):
    ...
    with torch.no_grad():
        ...
        save_img_before_nms, which_dataset, save_label_car_tool_format_txt, \
        save_vehicle8cls_IOTMOTC_format_txt, save_dota_txt = \
            'vehicle8cls_detect', source_path, \
            [str(weight_file_path)], \
            False, False, 1920, str(cfg_file_path), \
            str(cls_file_path), 'dhxdhy', False, 'vehicle8cls', \
            False, True, False
        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            # Apply NMS
            # pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms, num_extra_outputs=num_extra_outputs)
            pred = rotate_non_max_suppression(pred, conf_thres, iou_thres, classes=[], agnostic=agnostic_nms, \
                                                theta_format=theta_format, num_extra_outputs=num_extra_outputs, whichdataset=which_dataset)
            t2 = time_synchronized()
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in det:  # xyxy: x1y1x2y2x3y3x4y4 clockwise
                        ...
                        if save_label_car_tool_format_txt:
                            ...
                            # label_it_car的角點順序是左上角點逆時針 x1y1x2y2x3y3x4y4 -> x1y1x4y4x3y3x2y2
                            pts_roll = torch.roll(xyxy.view(-1,2), -1, 0).flip(0).view(-1)
                            with open(save_path[:save_path.rfind('.')] + '.txt', 'a', encoding='utf-8') as file:
                                # file.write(('%g ' * 5 + '\n') % (obj_cls, *xywh))  # label format
                                file.write('{} {} {} {} {} {} {} {} {} {} {} {} {} {}\n'.format(int(cls), *xywh, *pts_roll, out_theta))  # GUI label format
                        ...
                        if save_dota_txt:
                            ...
                            with open(dota_detect_cls_txt_dir+os.sep+"Task1_"+names[int(cls)]+".txt", "a+", encoding='utf-8') as file:
                                file.write(out_dota_str)

                        if save_img or view_img:  # Add bbox to image
                            label = '%s %.2f' % (names[int(cls)], conf)
                            draw_one_polygon(im0, xyxy, int(cls), line_thickness=2)
                            # plot_one_box(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=3)
                ...
                if save_img:
                    ...
                    if dataset.mode == 'images':
                        if os.name == 'nt':
                            cv2.imencode(ext='.png',img=im0)[1].tofile(save_path)
                        else:
                            cv2.imwrite(save_path, im0)
                        if save_img_before_nms:
                            if os.name == 'nt':
                                cv2.imencode(ext='.png',img=img_before_nms)[1].tofile(before_nms_save_path)
                            else:
...
#     # /media/lab602/1C1051B41051959C/IoT_MOTC_video/宜蘭G1_stab.avi
#     # --source /media/lab602/1C1051B41051959C/IoT_MOTC_video/宜蘭G1_stab.avi --output vehicle8cls_detect_高雄市前鎮區二聖一路_光華二路口80米_C_stab --which-dataset vehicle8cls --theta-format dhxdhy --conf-thres 0.3 --iou-thres 0.1 --img-size 1920 --device 1 --agnostic-nms --names /home/lab602/lin_data/Experiment_data/data/vehicle8cls/classes_vehicle8cls.txt --cfg '/home/lab602/lin_data/Experiment_data/data/vehicle8cls/yolov4-pacsp-mish-9anchor-headcxy-vehicle8cls-1920_w_layerID_wo_whrepair.cfg'  --weights /media/lab602/1C1051B41051959C/PyTorch_YOLOv4/vehicle8cls/exp8_20211109172733-yolov4-pacsp-mish-9anchor-headcxy-vehicle8cls-1920_w_layerID_wo_whrepair-dhxdhy-rioudhxdhy-image_weights-scratch--cosine-1920/weights/last_200_1920.pt' --save_vehicle8cls_IOTMOTC_format_txt
...
#     parser.add_argument('--save_label_car_tool_format_txt', action='store_true', help='save results in label_it_car format to *.txt. All results in one txt file')
#     parser.add_argument('--save_vehicle8cls_IOTMOTC_format_txt', action='store_true', help='save results in IoT MOTC (運研所) format to *.txt. All results in one txt file')
#     parser.add_argument('--save_dota_txt', action='store_true', help='save results in DOTA1.0 format to *.txt.')
