...
from utils_pkg.datasets import create_dataloader
from utils_pkg.general import (
    coco80_to_coco91_class, check_file, check_img_size, compute_loss, non_max_suppression,
    scale_coords, xyxy2xywh, clip_coords, plot_images, xywh2xyxy, box_iou, r_box_iou, output_to_target)
from utils_pkg.torch_utils import select_device, time_synchronized

from models_pkg.models import *
#from utils.datasets import *

from utils_pkg.metrics import ConfusionMatrix, ap_per_class
...
def test(data,
    ...
    if training:  # called by train.py
        ...
        with open(Path(save_dir) / 'opt.yaml', 'w', encoding='utf-8') as f:
    ...
    for batch_i, (img, targets, paths, shapes, cv_imgs) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            if training:  # if model has loss hyperparameters
                loss += compute_loss([x.float() for x in train_out], targets, model, -1, -1, -1, theta_format, loss_terms, num_extra_outputs, n_loss_terms, use_angle_modify_factor=angle_modify_factor)[1][:n_loss_terms]  # GIoU, obj, cls
            ...
            # 在training時，每結束一個epoch會valid一次，每valid一次就存batch 0的第一張圖存到tensorboard
            if training and batch_i == 0:
    ...
    # Save results
    if not training:
        with open(Path(save_dir) / 'val_results.txt', 'w', encoding='utf-8') as outfile:
            outfile.write(s+'\n')
