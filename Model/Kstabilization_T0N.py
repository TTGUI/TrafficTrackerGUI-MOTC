...
class StableVideo:
    def __init__(self, inputVideoPath, cut_txt, output_width, output_height):
        ...
        if cap.isOpened():
            ...
            self.target_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))  # get width from origin video
            self.target_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))  # get height from origin video
    ...
    def stableVideoWithOutputpath(self, outputpath, display_callBack, is_Show=False ):
        ...
        while index < len(self.videolist):
            if self.cutInfoList[index].getKey() != -1:
                ...
                frameT = cv2.resize(frame, (self.output_width, self.output_height), interpolation=cv2.INTER_CUBIC)
        ...
        out = cv2.VideoWriter(outputpath, self.fourcc, self.output_fps, (self.output_width, self.output_height))
        ...
        while self.currentVideoIndex < len(self.videolist):
            while (self.cutInfoList[self.currentVideoIndex].getKey() == -1 and
                   self.cutInfoList[self.currentVideoIndex].getStart() == -1 and
                   self.cutInfoList[self.currentVideoIndex].getEnd() == -1):
            ...
            total_frames = int(self.cutInfoList[self.currentVideoIndex].getEnd() - self.cutInfoList[self.currentVideoIndex].getStart())
            for _ in tqdm(range(total_frames), desc=f"Processing {self.videolist[self.currentVideoIndex]}"):
                self.IO_S = time.time()
                ret, frame = cap.read()
                self.IO_E = time.time()
    ...
    def cuttingData(self, cut_txt):
        ...
        for i in range(len(self.cutInfoList)):
            print(f"Key : {self.cutInfoList[i].getKey()} \t Start : {self.cutInfoList[i].getStart()} \t End : {self.cutInfoList[i].getEnd()}")
        print("=======================================================")
