...
from Model.sort7 import iou_rotated
...
def min_distance(r1, r2):
    ...
    return min_rotation, min_rotation_idx
...
def del_outliers(quadrilateral_positions):
    ...
    i = 0
    for i in range(n):
        if np.all(quadrilateral_positions[i] == 0):
            continue
        if i > 0:
            diff_angle_prev = abs(angles[i] - angles[i-1])
            diff_angle_prev = min(360 - diff_angle_prev, diff_angle_prev)
            if diff_angle_prev > 15 or np.all(quadrilateral_positions[i-1] == 0):  # 使用 numpy 的 all() 來檢查陣列中的所有元素
        ...
        if i < n-1:
            ...
            if diff_angle_next > 15 or np.all(quadrilateral_positions[i+1] == 0):
                new_positions[i] = [0, 0, 0, 0, 0, 0, 0, 0]
    ...
    with open(tracking_csv, 'w', newline='') as csvfile:
        ...
        for id, data in track2.items():
            if len(data[11]) > 20:
                ...
                traj = np.array(del_outliers(reorder_data)).reshape(-1, 8)
