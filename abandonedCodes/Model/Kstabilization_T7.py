...
class StableVideo:
    ...
    def stableVideoWithOutputpath(self, outputpath, is_Show=False):
        logger.info("[Step 1] ->> Get Target Frame.")
        # get target frame
        index = 0
            ...
            while self.currentVideoIndex < len(self.videolist) :
                ...
                while self.currentFrameIndex < self.cutInfoList[self.currentVideoIndex].getEnd() :
                    ...
                    if len(unit) == self.poolSize or self.currentFrameIndex == self.cutInfoList[self.currentVideoIndex].getEnd()-1: 
                        ...
                        ECC_S = time.time()
                        result = pool.imap_unordered(self.ECC, unit)
                        ECC_E = time.time()
                        self.ECC_costTime = self.ECC_costTime = (ECC_E - ECC_S)

                        
                        result_S = time.time()
                        
                        for ret in result:
                            frame = cv2.resize(ret, (1920, 1080), interpolation=cv2.INTER_CUBIC)
                            out.write(frame) 
                        result_E = time.time()
                        self.Write_costTime = self.Write_costTime + (result_E - result_S)   
                        unit = []
                        self.IO_S = time.time()
